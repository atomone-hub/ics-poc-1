...
func (v *validator) init() error {
	...
	var appState []byte
	if strings.Contains(v.chain.id, "consumer") {
		appState, err = json.MarshalIndent(consumerApp.ModuleBasics.DefaultGenesis(v.chain.cdc), "", " ")
	}else{
		appState, err = json.MarshalIndent(providerApp.ModuleBasics.DefaultGenesis(v.chain.cdc), "", " ")
}
